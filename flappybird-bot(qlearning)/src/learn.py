...
def showGameOverScreen(crashInfo):
    ...
    if VERBOSE:
        with open("data/highscore&deathcount.txt") as dfile:
            tries = dfile.readlines()
        print(tries[1] + " | " + str(score))
    with open("data/highscore&deathcount.txt", "r") as hdfile:
        highscore = int(hdfile.readline().strip())
        deaths = int(hdfile.readline().strip())
        if score > highscore:
            highscore = score
            print(f"Highscore: {highscore}, tries: {deaths}")
        deaths += 1
    with open("data/highscore&deathcount.txt", "w") as hdfile:
        hdfile.write(f"{highscore}\n")
        hdfile.write(f"{deaths}")
